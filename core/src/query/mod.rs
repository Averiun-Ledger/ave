...
use ave_common::{response::{LedgerDB, PaginatorAborts, PaginatorEvents, SubjectDB, TimeRange}};
...
impl Actor for Query {
    ...
    fn get_span(id: &str, parent_span: Option<Span>) -> tracing::Span {
        if let Some(parent_span) = parent_span {
            info_span!(parent: parent_span, "Query", id = id)
        } else {
            info_span!("Query", id = id)
        }
    }
    ...
    async fn handle_message(
        ...
        match msg {
            QueryMessage::GetEvents { subject_id, quantity, page, reverse, event_request_ts, event_ledger_ts, sink_ts } => {
                match self.db.get_events(&subject_id, quantity, page, reverse, event_request_ts, event_ledger_ts, sink_ts).await {
            ...
            QueryMessage::GetAborts { subject_id, request_id, sn, quantity, page, reverse } => {
                match self.db.get_aborts(&subject_id, request_id, sn, quantity, page, reverse).await {
                    Ok(data) => Ok(QueryResponse::PagAborts(data)),
                    Err(e) => Ok(QueryResponse::Error(e.to_string()))
                }
            },
            QueryMessage::GetEventSn { subject_id, sn } => {
                match self.db.get_event_sn(&subject_id, sn).await {
                    Ok(data) => Ok(QueryResponse::Event(data)),
                    Err(e) => Ok(QueryResponse::Error(e.to_string()))
                }
            },
            QueryMessage::GetFirstOrEndEvents { subject_id, quantity, reverse } => {
                match self.db.get_first_or_end_events(&subject_id, quantity, reverse).await {
                    Ok(data) => Ok(QueryResponse::Events(data)),
                    Err(e) => Ok(QueryResponse::Error(e.to_string()))
                }
            },
            QueryMessage::GetSubject { subject_id } => {
                match self.db.get_subject_state(&subject_id).await {
