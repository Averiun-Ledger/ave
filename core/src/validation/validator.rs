...
use ave_common::identity::{DigestIdentifier, PublicKey, TimeStamp};
...
const TARGET_VALIDATOR: &str = "Ave-Validation-Validator";
...
impl Validator {
    pub fn new(request_id: String, version: u64, node: PublicKey) -> Self {
        Validator {
            request_id,
            version,
            node,
    ...
    async fn check_proofs(
        ...
        if let Some(previous_proof) = previous_proof {
            ...
            if previous_proof.governance_version == gov.version {
                ...
                let previous_signers: Result<HashSet<PublicKey>, ActorError> =
