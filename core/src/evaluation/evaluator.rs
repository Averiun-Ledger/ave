...
const TARGET_EVALUATOR: &str = "Ave-Evaluation-Evaluator";
            ...
            if let Some(compiler_actor) = compiler_actor {
                compiler_actor
                    .ask(CompilerMessage::Compile {
                        ...
                        contract_path: 
                        config.contracts_dir.join("contracts").join(format!("{}_{}", governance_id, id))
                    })
                    .await?
            } else {
            ...
            EventRequest::Transfer(transfer_event) => {
                if !is_governance {
                    (
                        ...
                        evaluation_req.gov_state_init_state.clone(),
                    )
                } else {
                    (
