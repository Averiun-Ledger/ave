...
use common::{
    ...
    emit_fact_viewpoints, emit_reject, emit_transfer, get_events,
...
async fn test_viewpoints_transfer_override_battery() {
    ...
    let pending_events =
        get_events(new_owner, subject_id.clone(), 4, true)
            .await
            .unwrap();
    assert_tracker_fact_full(
    ...
    let confirmed_events =
        get_events(new_owner, subject_id.clone(), 5, true)
            .await
            .unwrap();
    assert_tracker_fact_full(
    ...
    let final_events =
        get_events(final_owner, subject_id.clone(), 6, true)
            .await
            .unwrap();
    assert_tracker_fact_full(
...
async fn test_viewpoints_reject_and_old_owner_battery() {
    ...
    old_rejected
        ...
        .await
        .unwrap();
    let _state = get_subject(&old_rejected, governance_id.clone(), Some(1), true)
        .await
        .unwrap();
    ...
    old_rejected
        ...
        .await
        .unwrap();
    old_rejected.update_subject(subject_id.clone()).await.unwrap();
    ...
    old_owner
        .auth_subject(
            governance_id.clone(),
            AuthWitness::One(PublicKey::from_str(&witness_full.public_key()).unwrap()),
        )
        .await
        .unwrap();
    old_owner
    ...
    let invalid_events = get_events(&invalid_witness, subject_id.clone(), 9, true)
        .await
        .unwrap();
