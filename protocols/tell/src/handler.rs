    ...
    TCodec: Codec,
{
    ...
    worker_streams: futures_bounded::FuturesMap<
        TellId,
        Result<TellEvent<TCodec>, io::Error>,
    >,
}
    ...
    TCodec: Codec + Send + Clone + 'static,
{
    ...
    fn on_fully_negotiated_outbound(
        ...
        let send = async move {
            let write =
                codec.write_message(&protocol, &mut stream, message.message);
            write.await?;
            stream.close().await?;
        ...
        {
            tracing::warn!(
                "Dropping outbound stream because we are at capacity"
            )
        }
    }
        ...
        match error {
            ...
            StreamUpgradeError::NegotiationFailed => {
                ...
                self.pending_events.push_back(
                    TellEvent::OutboundUnsupportedProtocols(message.tell_id),
                );
            }
...
impl<TCodec: Codec> fmt::Debug for TellEvent<TCodec> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TellEvent::TellProccessed(id) => {
                write!(f, "TellProccessed({})", id)
            }
        }
    }
}
