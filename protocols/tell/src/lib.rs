    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            OutboundFailure::Io(e) => {
                write!(f, "IO error on outbound stream: {e}")
            }
        }
    }
}
    ...
    TCodec: Codec + Clone + Send + 'static,
{
    ...
    pending_events:
        VecDeque<ToSwarm<Event<TCodec::Message>, OutboundMessage<TCodec>>>,
    ...
    pending_outbound_messages:
        HashMap<PeerId, SmallVec<[OutboundMessage<TCodec>; 10]>>,
}
    ...
    TCodec: Codec + Clone + Send + 'static,
{
    ...
    pub fn addresses_of_peer(
        &mut self,
        peer: &PeerId,
    ) -> impl Iterator<Item = Multiaddr> + '_ {
    ...
    fn preload_new_handler(
        ...
        if let Some(pending_messages) =
            self.pending_outbound_messages.remove(&peer)
        {
    ...
    TCodec: Codec + Send + Clone + 'static,
{
    ...
    fn handle_pending_outbound_connection(
        ...
        if let Some(connections) = self.connected.get(&peer) {
            addresses.extend(
                connections.iter().filter_map(|c| c.remote_address.clone()),
            )
        }
        Ok(addresses)
    }
    ...
    fn poll(
        ...
        ToSwarm<Self::ToSwarm, libp2p::swarm::THandlerInEvent<Self>>,
    > {
