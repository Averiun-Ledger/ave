...
async fn test_get_sinks_returns_list() {
    ...
    let (status, body) =
        make_request(&client, &server.url("/sinks"), "GET", None, None).await;

    assert_eq!(status, 200);
    let sinks: Value =
        serde_json::from_value(body).expect("response should be JSON");
...
async fn test_get_sinks_filters_by_name() {
    ...
    assert_eq!(status, 200);
    let sinks: Vec<Value> =
        serde_json::from_value(body).expect("response should be JSON array");
