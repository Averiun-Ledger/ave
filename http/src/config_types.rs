...
// Every level is destructured without `..` on purpose: a new field in the
// common sink config must fail compilation here instead of being silently
// dropped from the `/config` view.
impl From<ave_bridge::SinkServer> for SinkServerHttp {
    fn from(value: ave_bridge::SinkServer) -> Self {
        use ave_bridge::ave_common::sink::{
            GrpcSinkConfig, GrpcTlsConfig, HttpProxyConfig, HttpSinkConfig,
            HttpTlsConfig, KafkaSinkConfig, KafkaTlsConfig,
        };

        let ave_bridge::SinkServer {
            server,
            events,
            transport,
            catch_up_batch_size,
            batch_delivery_size,
            sink_worker_idle_timeout_ms,
            healthcheck_intervals_secs,
            max_catch_up_concurrency,
            sink_subject_worker_idle_timeout_ms,
            max_recoveries_after_failure,
        ...
        let transport = match transport {
            ave_bridge::SinkTransportConfig::Http(http) => {
                ...
                SinkTransportConfigHttp::Http(Box::new(HttpSinkConfigHttp {
                    url,
                    auth: auth.map(sink_auth_method_to_http),
                    connect_timeout_ms,
                    request_timeout_ms,
                    max_retries,
                    retry_base_delay_ms,
                    tls: tls.map(|t| {
                        ...
                        HttpTlsConfigHttp {
                            ...
                            client_key,
                            pinned_certificate,
                            min_tls_version: min_tls_version.map(Into::into),
                        }
                    }),
                    signature,
                    signature_version,
            ...
            ave_bridge::SinkTransportConfig::Kafka(kafka) => {
                ...
                SinkTransportConfigHttp::Kafka(Box::new(KafkaSinkConfigHttp {
                    ...
                    topic,
                    client_id,
                    security: kafka_security_to_http(security),
                    tls: tls.map(|t| {
                        let KafkaTlsConfig {
                            ca_certificate,
                            client_certificate,
                    ...
                    oauth_token_url,
                    oauth_scope,
                    kerberos: kerberos.map(Into::into),
                }))
            }
        ...
        Self {
            server,
            events: events.into_iter().map(|e| e.to_string()).collect(),
            transport,
            catch_up_batch_size,
            batch_delivery_size,
            sink_worker_idle_timeout_ms,
